package br.com.cadernetadigital.component.lista;
...
public class Lista extends UINamingContainer {
	...
	public String getClientId(FacesContext faces) {
		...
		if (this.index >= 0) {
			id = this.getBuffer().append(id).append(getSeparatorChar(faces))
	...
	private void captureOrigValue(FacesContext ctx) {
		if (this.var != null || this.varStatus != null) {
			Map<String, Object> attrs = ctx.getExternalContext()
					.getRequestMap();
			if (this.var != null) {
	...
	private void restoreChildState(FacesContext faces, UIComponent c) {
		...
		if (c instanceof EditableValueHolder) {
			...
			if (ss != null) {
				...
				String childId = clientId
						.substring(initialClientId.length() + 1);
				childId = childId.substring(childId
						.indexOf(getSeparatorChar(faces)) + 1);
	...
	private boolean isNestedInIterator() {
		...
		while (parent != null) {
			if (parent instanceof UIData || parent instanceof Lista) {
				return true;
			}
	...
	/**
	 * Função responsável por renderizar os elementos dentro do loop.
	 * 
	 * @param faces
	 * @param phase
	 */
	public void process(FacesContext faces, PhaseId phase) {
		...
		// We must clear the child state if we just entered the Render Phase,
		...
		try {
			...
			if (this.getChildCount() > 0) {
				...
				while (i <= e && this.isIndexAvailable()) {
					...
					i += s;
					this.setIndex(faces, i);
					this.updateIterationStatus(faces, new IterationStatus(
							false, i + s >= e, i, begin, end, step));
				}
			}
		} catch (IOException e) {
		...
		/*
		 * Once rendering is done we need to make sure the child components are
		 * not still having client ids that use an index.
		 */
		if (PhaseId.RENDER_RESPONSE.equals(phase)) {
			resetClientIds(this);
		}
	...
	public boolean invokeOnComponent(FacesContext faces, String clientId,
		...
		if (clientId.equals(id)) {
			...
			int idxStart = clientId.indexOf(getSeparatorChar(faces),
					id.length());
			if (idxStart != -1
	...
	private void validateIterationControlValues(int rowCount, int begin, int end) {
		...
		if (begin > rowCount) {
			throw new FacesException(
					"Iteration start index is greater than the number of available rows.");
		}
		if (begin > end) {
			throw new FacesException(
					"Iteration start index is greater than the end index.");
		}
		if (end > rowCount) {
			throw new FacesException(
					"Iteration end index is greater than the number of available rows.");
		}
	}
	...
	private boolean visitChildren(VisitContext context, VisitCallback callback) {
		...
		this.setIndex(faces, i);
		this.updateIterationStatus(faces, new IterationStatus(true,
				(i + s > e || rowCount == 1), i, begin, end, step));
		while (i < e && this.isIndexAvailable()) {

			this.setIndex(faces, i);
			this.updateIterationStatus(faces, new IterationStatus(false,
					i + s >= e, i, begin, end, step));
			for (UIComponent kid : getChildren()) {
				if (kid.visitTree(context, callback)) {
					return true;
				}
	...
	private static final class IndexedEvent extends FacesEvent {
		...
		public IndexedEvent(Lista owner, FacesEvent target, int index) {
			super(owner);
			this.target = target;
			this.index = index;
		}
		...
		public void processListener(FacesListener listener) {
			Lista owner = (Lista) this.getComponent();
	...
	public void broadcast(FacesEvent event) throws AbortProcessingException {
		if (event instanceof IndexedEvent) {
			...
			try {
				...
				int e = ((end != null) ? end : rowCount);
				int s = ((step != null) ? step : 1);
				this.updateIterationStatus(ctx, new IterationStatus(idx == b,
						(idx + s >= e || rowCount == 1), idx, begin, end, step));
				if (this.isIndexAvailable()) {
